// TabControlHelper.cs, version 1.2
...
namespace Com.Ericmas001.Windows.Wpf.Helpers
{
    /// <summary>
    ...
    /// Set ikriv:TabControlHelper.IsCached="True" to preserve visual state of each tab.
    /// </remarks>
    public static class TabControlHelper
    {
        ...
        /// </summary>
        /// <remarks>When TabControlHelper.IsCached is true, visual state of each tab is preserved (cached), even when the tab is hidden</remarks>
        public static readonly DependencyProperty IsCachedProperty =
            DependencyProperty.RegisterAttached("IsCached", typeof(bool), typeof(TabControlHelper), new UIPropertyMetadata(false, OnIsCachedChanged));
        ...
        public static readonly DependencyProperty TemplateProperty =
            DependencyProperty.RegisterAttached("Template", typeof(DataTemplate), typeof(TabControlHelper), new UIPropertyMetadata(null));
        ...
        public static readonly DependencyProperty TemplateSelectorProperty =
            DependencyProperty.RegisterAttached("TemplateSelector", typeof(DataTemplateSelector), typeof(TabControlHelper), new UIPropertyMetadata(null));
        ...
        public static readonly DependencyProperty InternalTabControlProperty =
            DependencyProperty.RegisterAttached("InternalTabControl", typeof(TabControl), typeof(TabControlHelper), new UIPropertyMetadata(null, OnInternalTabControlChanged));
        ...
        public static readonly DependencyProperty InternalCachedContentProperty =
            DependencyProperty.RegisterAttached("InternalCachedContent", typeof(ContentControl), typeof(TabControlHelper), new UIPropertyMetadata(null));
        ...
        public static readonly DependencyProperty InternalContentManagerProperty =
            DependencyProperty.RegisterAttached("InternalContentManager", typeof(object), typeof(TabControlHelper), new UIPropertyMetadata(null));
        ...
        private static void OnIsCachedChanged(DependencyObject obj, DependencyPropertyChangedEventArgs args)
        {
            ...
            if (tabControl == null)
            {
                throw new InvalidOperationException("Cannot set TabControlHelper.IsCached on object of type " + args.NewValue.GetType().Name +
                    ". Only objects of type TabControl can have TabControlHelper.IsCached property.");
            }
        ...
        {
            const string xaml =
                "<DataTemplate><Border b:TabControlHelper.InternalTabControl=\"{Binding RelativeSource={RelativeSource AncestorType=TabControl}}\" /></DataTemplate>";

            var context = new ParserContext();

            context.XamlTypeMapper = new XamlTypeMapper(new string[0]);
            context.XamlTypeMapper.AddMappingProcessingInstruction("b", typeof(TabControlHelper).Namespace, typeof(TabControlHelper).Assembly.FullName);
        ...
        private static void OnInternalTabControlChanged(DependencyObject obj, DependencyPropertyChangedEventArgs args)
        {
            ...
            if (container == null)
            {
                var message = "Cannot set TabControlHelper.InternalTabControl on object of type " + obj.GetType().Name +
                    ". Only controls that derive from Decorator, such as Border can have a TabControlHelper.InternalTabControl.";
            ...
            {
                throw new InvalidOperationException("Value of TabControlHelper.InternalTabControl cannot be of type " + args.NewValue.GetType().Name + ", it must be of type TabControl");
            }
        ...
        private static void EnsureContentTemplateIsNull(TabControl tabControl)
        {
            ...
            {
                throw new InvalidOperationException("TabControl.ContentTemplate value is not null. If TabControlHelper.IsCached is True, use TabControlHelper.Template instead of ContentTemplate");
            }
        }
        ...
        private static void EnsureContentTemplateIsNotModified(TabControl tabControl)
        {
            ...
            {
                throw new InvalidOperationException("Cannot assign to TabControl.ContentTemplate when TabControlHelper.IsCached is True. Use TabControlHelper.Template instead");
            });
        }
        ...
        public class ContentManager
        {
            ...
            private ContentControl GetCurrentContent()
            {
                ...
                var cachedContent = TabControlHelper.GetInternalCachedContent(tabItem);
                if (cachedContent == null)
                {
                    cachedContent = new ContentControl
                    {
                        DataContext = item,
                        ContentTemplate = TabControlHelper.GetTemplate(_tabControl),
                        ContentTemplateSelector = TabControlHelper.GetTemplateSelector(_tabControl)
                    };

                    cachedContent.SetBinding(ContentControl.ContentProperty, new Binding());
                    TabControlHelper.SetInternalCachedContent(tabItem, cachedContent);
                }
